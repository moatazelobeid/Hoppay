...
<article class="module width_full">
    <header><h3 class="tabs_involved">Email Template List</h3>
    <a href="<?=$this->webroot?>admin/newsletter/add_email_template" class="heading_link">Add Email Template</a>
    </header>
 ...
 <form method="get" action="<?=$this->webroot?>admin/newsletter/email_template"> 
        ...
        <input type="submit" class="search_button" name="search" value="Search" placeholder="Search by here."> 
            ...
            <td width="1%">
               <a class="reset_button" href="<?=$this->webroot?>admin/newsletter/email_template" class="reset">Reset</a> 
           </td>
         </tr>
      </table>
  </form> 
      ...
      <thead> 
        <tr> 
            ...
            <th align="" width="100"><?php echo $this->Paginator->sort('Email_template.email_type', 'Title'); ?></th> 
            <th width="250"><?php echo $this->Paginator->sort('Email_template.email_subject', 'Email Subject'); ?></th>            
      ...
      <tbody> 
        ...
        <?php } else {?>
       <?php foreach($email_template_list as $key=>$val) { ?>
        <tr> 
            <td>
          
           <input type="checkbox" data-id="<?=$val['Email_template']['id']?>" class="user_checked <?=$val['Email_template']['id']?>">
     
            </td> 
            <td><?=htmlspecialchars_decode($val['Email_template']['email_type'])?></td>        
            <td><?=htmlspecialchars_decode($val['Email_template']['email_subject'])?></td>        
            
  
            <td><center><?=$val['Email_template']['status']?'<p style="color:#408080">Active<p>':'<p style="color:#f01">Inactive<p>'?></center></td> 
            <td>
              <a href="<?=$this->webroot?>admin/newsletter/update_email_template/<?=$val['Email_template']['id']?>/<?=@$this->params['named']['page']?>"><input type="image" src="<?=$this->webroot?>images/dashbord/icn_edit.png" title="Edit"></a>
               
              <a onclick="deleteAction('<?=$this->webroot?>admin/newsletter/delete/<?=$val['Email_template']['id']?>/<?=@$this->params['named']['page']?>','<?=$menu_title?>','<?=htmlspecialchars_decode($val['Email_template']['email_type'])?>',false,'Are you want to delete this EMAIL TEMPLATE LIST?')" href="javascript:void(0)">   <input type="image" src="<?=$this->webroot?>images/dashbord/icn_trash.png" title="Trash"></a>
            </td> 
