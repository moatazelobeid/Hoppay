...
function getProducts(prod)
{
  if(prod!='')
  {
    var url = '<?php echo $this->webroot;?>admin/getReviewProducts/'+prod+'/<?=@$this->request["url"]["merchantid"]?>';  
    //alert(url);
...
if(data.length<=0){
  //alert('no data');
    ...
    if($(this).val()=='1')
    {
        $.post('<?=$this->webroot?>admin/bulk_active',{'ids':JSON.stringify(jsonArray),'model':'Product_review'},function(r){
          //console.log(r);
          if(r==1)
          {
    ...
    {
      $.post('<?=$this->webroot?>admin/bulk_inactive',{'ids':JSON.stringify(jsonArray),'model':'Product_review'},function(r){
         // console.log(r);
          if(r==1)
          {
    ...
    {
      $.post('<?=$this->webroot?>admin/bulk_delete',{'ids':JSON.stringify(jsonArray),'model':'Product_review'},function(r){
          //console.log(r);
          if(r==1)
          {
    ...
    {
      $.post('<?=$this->webroot?>admin/bulk_add_slider',{'ids':JSON.stringify(jsonArray),'model':'Product_review'},function(r){
          console.log(r);
          if(r=='1')
          {
    ...
    {
      $.post('<?=$this->webroot?>admin/bulk_remove_slider',{'ids':JSON.stringify(jsonArray),'model':'Product_review'},function(r){
          console.log(r);
          if(r=='1')
          {
  ...
  <!--  <a href="<?=$this->webroot?>admin/add_merchant" class="heading_link">Add Merchant</a> -->
     ...
     <div class="module_content listing_containt">
       <div id="stylized" class="myform search" style="width:100%">
         <table style="width:00%;float:left">
          <tr>
            <td> 
 ...
 <form method="get" action="<?=$this->webroot?>admin/ratingReviews"> 
      ...
      <thead> 
        <tr> 
            ...
            <th width="100px"><?php echo $this->Paginator->sort('rating', 'Ratings'); ?></th> 
