			...
			if(!empty($offer))
			{
				if(($offer['status'] == 1) && ($offer['end_date']!='0000-00-00 00:00:00') && ($offer['end_date'] >= $cdate)) 
				{
					if(!empty($offer['discount']))
					{
						...
						$result .= '<span>'.$this->getPriceFormat(number_format($price,2)).'</span>'; 	
...
public function GetProductCountBycategory($cat_id="",$search="",$type=""){
  ...
  }else
  {
   ...
   if(!empty($results))
  {
	 $count=$this->Product->find('count',array('conditions'=>array('Product.category_id !='=>"",'Product.category_id'=>$results,'Product.status'=>1)));
...
public function getMerchantDetailsByProduct($id="",$slug="",$type="count",$lang_id=1)/*(all|count)*/
{
  ...
  $merchant=$this->Product->find($type,array('conditions'=>array('Product.slug like'=>$slug."%",'Product.category_id'=>$product['Product']['category_id'],'Product.brand'=>$product['Product']['brand'])));
    ...
    public function getProductMerchantCount($id="",$slug="",$retailer_id="",$type="count",$lang_id=1)/*(all|count)*/
	{
	  $this->Product = ClassRegistry::init('Product');
  ...
  public function getSellerText($count){
    ...
    $this->Product_category = ClassRegistry::init('Product_category');
		...
		$lang_nm = $this->getLang(); //echo $lang_id;
			if($lang_nm == 'en'){
				$lang_id = 1;
				$order = 'desc';
			}else if($lang_nm == 'ar'){
    ...
    $cats=$this->Product_category->find('all',array('conditions'=>array('Product_category.status'=>1,'Product_category.parent_id'=>0),'order'=>array('Product_category.cat_order'=>$order)));
 ...
 $child_ids=Hash::extract($child_cats,'{n}.Product_category.id'); //print_r($child_ids);
        ...
        <li>
          <?php 
                $offer=$this->Product->find('all',
                  array(
                    ...
                    'conditions'=>array(
					  'Offer.offer_title !='=>'0.00%. Discount',
                      'Offer.start_date <= ' => $date,
                      'Offer.end_date  >= ' => $date,
                      'Offer.status' => 1,
                      'Product.category_id'=>$child_ids,
            ...
            if(!empty($offer))
          {
            ...
            foreach ($offer as $key => $value) 
            {
			  ...
			  //echo "<pre>";print_r($product_lang_data);
			 //if($product_lang_data['']){
              ...
              <li>
				...
				<div class="g-offers">
					...
					<b>
						<?php
							echo "<div style='text-decoration:line-through;'>".$this->getPriceFormat(number_format($value['Product']['price'],2))."</div>";
							echo "<div>".$this->getPriceFormat(number_format($offer_price,2))."</div>";
						?>	
					</b><br>
					<?php if($value['Offer']['offer_image']!=""){?><!--<img src="<?=$this->webroot?><?=$value['Offer']['offer_image']?>" style="height:30px;float:left;margin-top:1px;width:auto" alt="hoppay">--><?php } ?>
				</div>
                </div>
                </a>
              </li>
                ...
                <div class="gimage">
                    <a href="<?=$this->webroot.$this->getLang()?>/homes/offers">
                      <!-- <div style="height:20px"></div>-->
                          <div class="img_cover">
                        <?php echo $this->Html->image('seeall.png', array('alt' => 'hoppay'));?>
                    </div>
                    </a>
    ...
    <?php /* ?></ul>
        <ul>
            ...
            //print_r($offer);
                  ...
                  <div style="height:20px"></div>
							 <div class="g-offers"><span class="dsntnew1"><?=$value['Offer']['offer_desc']?></span><br><s><?=$this->getPriceFormat(number_format($value['Product']['price'],2));?></s><br /><b><?=$this->getPriceFormat(number_format($offer_price,2));?></b><br><?php if($value['Offer']['offer_image']!=""){?><!--<img src="<?=$this->webroot?><?=$value['Offer']['offer_image']?>" style="height:30px;float:left;margin-top:1px;width:auto" alt="hoppay">--><?php } ?></div>
								</div>
								</a>
							</li>
							<?php 
...
$url=$this->here;
  ...
  public function getBrandOnTop(){
	...
	$product_brand_tot = $this->Product->Find('list', array('conditions'=>array('Product.status'=>1),'fields'=>array('Product.brand')));
	$brand_ids = array_unique($product_brand_tot);
     $product_brands = $this->Product_brand->Find('all', 
                    array(
                    'conditions'=>array(
                      'Product_brand.status'=>1,
					  'Product_brand.id'=>$brand_ids
                    ),
                    'order' => array('Product_brand.order'=>'asc'),
                    'limit'=>13
                  )
                );  //echo "<pre/>";print_r($product_brands);exit;
      ?>
      <div class="grid">
                  <div class="listdata">
                      <ul>
              ...
              if(!empty($product_brands))
              {
                ...
                {
					//$cnt = count($product_brand['Product']); 
					//if($cnt > 0){
                 ...
                 $cat_ids=array_unique($cat_ids);
                  ...
                  ?>
                      ...
                      if(!empty($product_brand['Product_brand']['image_url']))
                      {
                                                ...
                                                <?php
                        echo $this->Html->image('../'.$product_brand['Product_brand']['image_url'], array('alt' => 'hoppay', 'class'=>'img_thumbnail_image'));
                                                ?>
                                                </a>
                                                <?php
                      }
                      ...
                      $brand_slug = $product_brand['Product_brand']['slug'];?>
                                        ...
                                        <div class="related_cat_over">
                                          <ul>
                                            ...
                                            if(count($cat_ids)<=0)
                                            {
                                              echo "<li class='not_found'>Nothing Found.</li>";
                                            }
                                            else
                                            {
              ...
              if(!empty($product_categories))
              {
                foreach($product_categories as $key=>$product_category)
                {
                     ...
                     //echo '<pre>'; print_r($product_categories); echo '</pre>'; exit;?>
                      ...
                      {
                        echo $this->Html->image('../'.$product_category['Product_category']['image_url'], array('alt' => 'hoppay', 'class'=>'img_thumbnail_image'));
                      }
                      else
                      {
                        echo $this->Html->image('no-image.png', array('alt' => 'hoppay', 'class'=>'img_thumbnail_image'));  
                      }?>
                                              </td></tr></table>
                                        </div>
      ...
      if($lang == 'en')
      {
        if(strlen($subcatname)>20)
          $subcatname = substr($subcatname,0,20).'...';
				...
				$parent_id = $subcat['Product_category']['parent_id'];
				?>
                    <a href="<?=$this->webroot.$lang?>/products/category-<?=$parent_id?>-dept-<?=$subcatslug?>" >
                        <?=$subcatname?>
                    </a>
                <?php
