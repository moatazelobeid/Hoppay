...
<article class="module width_full">
 ...
 <form method="get" action="<?=$this->webroot?>admin/merchant"> 
        ...
        <input type="submit" class="search_button" name="search" value="Search" placeholder="Search by here."> 
            ...
            <td width="1%">
               <a class="reset_button" href="<?=$this->webroot?>admin/merchant" class="reset">Reset</a> 
           </td>
         </tr>
      </table>
  </form> 
        ...
        <tr> 
         ...
         <?php  //print_r($val); ?>
           <input type="checkbox" data-id="<?=$val['Merchant_login']['id']?>" class="user_checked <?=$val['Merchant_login']['id']?>">
            ...
            <td><?=strip_tags(stripslashes($val['Merchant_login']['email_id']))?></td> 
            ...
            <a onclick="deleteAction('<?=$this->webroot?>admin/delete_merchant/<?=$val['Merchant_login']['id']?>','<?=$menu_title?>','<?=strip_tags(stripslashes($val['Profile']['first_name']))?>',false,'Are you want to delete this Merchant user?')" href="javascript:void(0)">   <input type="image" src="<?=$this->webroot?>images/dashbord/icn_trash.png" title="Trash"></a> 
             </td>
        </tr> 
        <?php } }?>
