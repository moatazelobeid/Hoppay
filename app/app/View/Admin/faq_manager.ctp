 ...
 $('#save_order').click(function(){
  ...
  var id=new Array();
    $('.get_order').each(function(){
     ...
     $.post('<?=$this->webroot?>admin/bulk_order',{'ids':JSON.stringify(jsonArrayids),'datas':JSON.stringify(jsonArraydata),'model':'Faq','field':'faq_order'},function(r){
          console.log(r);
          if(r=='1')
          {
...
if(data.length<=0){
  //alert('no data');
    ...
    if($(this).val()=='1')
    {
        $.post('<?=$this->webroot?>admin/bulk_active',{'ids':JSON.stringify(jsonArray),'model':'Faq'},function(r){
         // console.log(r);
          if(r==1)
          {
    ...
    {
      $.post('<?=$this->webroot?>admin/bulk_inactive',{'ids':JSON.stringify(jsonArray),'model':'Faq'},function(r){
         // console.log(r);
          if(r==1)
          {
    ...
    {
      $.post('<?=$this->webroot?>admin/bulk_delete',{'ids':JSON.stringify(jsonArray),'model':'Faq'},function(r){
          //console.log(r);
          if(r==1)
          {
...
<article class="module width_full">
    <header><h3 class="tabs_involved">FAQ List</h3>
 ...
 <form method="get" action="<?=$this->webroot?>admin/faq_manager"> 
        <table style="width:50%;float:right">
          <tr>
            <td width="2%">
        <input type="text" name="text_search" style="width:90px !important" 
        value="<?=(isset($this->request['url']['text_search']) and ($this->request['url']['text_search']!=""))?$this->request['url']['text_search']:''?>" placeholder="Search Here"> 
          ...
          <select name="cat_id" onchange="this.form.submit();" style="width: 80px;">
                      <option value="">--Choose Category--</option>
                        <?php foreach($faq_cat_data_all as $key=>$val) { ?> 
                        <option value="<?=$val['Faq_category_lang']['cat_id']?>" <?php echo $this->Template->Select($val['Faq_category_lang']['cat_id'],empty($cat_id)?"":$cat_id);?>><?=$val['Faq_category_lang']['category_name']?></option>
                       <?php } 
                        ?>
                      </select>
            ...
            <td width="2%">
            <select name="status" onchange="this.form.submit();" style="width:80px !important">
          ...
          <select name="lang_id" onchange="this.form.submit();" style="width: 43px;">
                        <?php 
                        foreach($lang as $val){ ?>
        ...
        <input type="submit" class="search_button" name="search" value="Search" placeholder="Search by here."> 
            ...
            <td width="1%">
               <a class="reset_button" href="<?=$this->webroot?>admin/faq_manager" class="reset">Reset</a> 
           </td>
         </tr>
      </table>
  </form> 
      ...
      <thead> 
        <tr> 
            ...
            <th width="400"><?php echo $this->Paginator->sort('Faq_lang.question', 'Questions'); ?></th>            
           
            <th><?php echo $this->Paginator->sort('Faq.category_id', 'Category'); ?></th> 
            <th align="" width="80"><?php echo $this->Paginator->sort('Faq.faq_order', 'Order'); ?> <img id="save_order" src="<?=$this->webroot?>/images/dashbord/save.png" style="float: right;margin-top:0px;margin-left: -7px;"></th> 
      ...
      <tbody> 
       ...
       //print_r($faq_info);
        ...
        <?php } else {?>
       <?php foreach($faq_info as $key=>$val) { ?>
        <tr> 
            <td>
           ...
           <input type="checkbox" data-id="<?=$val['Faq']['id']?>" class="user_checked <?=$val['Faq']['id']?>">
            ...
            <td><?=$val['Faq_lang']['question']?></td> 
            
            
            <td><?=$val['Faq_category_lang']['category_name']?></td> 
            <td><center><input type="text" class="get_order" data-id="<?=$val['Faq']['id']?>" value="<?=$val['Faq']['faq_order']?>" ></center></td>            
            <td><center><?=$val['Faq']['status']?'<p style="color:#408080">Active<p>':'<p style="color:#f01">Inactive<p>'?></center></td> 
            <td>
              <a href="<?=$this->webroot?>admin/update_faq/<?=$val['Faq']['id']?>"><input type="image" src="<?=$this->webroot?>images/dashbord/icn_edit.png" title="Edit"></a>
               
            <a onclick="deleteAction('<?=$this->webroot?>admin/delete_faq/<?=$val['Faq']['id']?>','<?=$menu_title?>','<?=$val['Faq_lang']['question']?>',false,'Are you want to delete this FAQ?')" href="javascript:void(0);">   <input type="image" src="<?=$this->webroot?>images/dashbord/icn_trash.png" title="Delete"></a></td> 
