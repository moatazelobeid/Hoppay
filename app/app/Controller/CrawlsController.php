  ...
  $row_id="";
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."'");  
                    ...
                    if(mysql_num_rows($rs1) > 0){
					   ...
					   $value1['category_name'] = preg_replace("/&#?[a-z0-9]+;/i","",$value1['category_name']);
					   
                       $value1['description'] = htmlspecialchars(strip_tags($value['category_name']));
					   $value1['description'] = preg_replace("/&#?[a-z0-9]+;/i","",$value1['description']);
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."'");  
                   ...
                   if(mysql_num_rows($rs1) > 0)
                   {
                    ...
                    $fcatid=$row1["id"];
                       ...
                       $value1['status']=1;
                       $value1['category_name'] = strip_tags(trim($value['category_name']));
					   $value1['category_name'] = mysql_real_escape_string($value1['category_name']);
                       $value1['description'] = htmlspecialchars(strip_tags($value['category_name']));
					   $value1['description'] = mysql_real_escape_string($value1['description']);
                       $this->loadmodel('Product_category_lang'); 
                       $this->Product_category_lang->create();
                       $this->Product_category_lang->save($value1);
                  } 
    ...
    $slug=$this->toAscii($department."-".$category_name.'-'.$product_name); 
    $cat_id=$fcatid;
	...
	$rs2 = mysql_query("select * from mc_products where product_url = '".$row["product_url"]."'")or die(mysql_error());
	if(mysql_num_rows($rs2) <= 0){
   
      mysql_query("insert into mc_products(slug,product_url,category_id,retailer_id,price,price_type,image_url,brand,status,deleted,created_date,offer_id) values(
	'".$slug."',
	'".$row["product_url"]."',
	'".$cat_id."','".$retailer_id."','".$price."','SAR',
	'".$row["image_url"]."','".$brand."',1,0,now(),'".$offer_id."')")or die(mysql_error());
	...
	'".$row["specification"]."','',1)")or die(mysql_error());
	...
	status=1,
	deleted=0,
	last_modified=now(),offer_id='".$offer_id."' where category_id = '".$cat_id."' and slug = '".$slug."' and retailer_id = '".$retailer_id."'")or die(mysql_error());
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."' and parent_id=0");  
                      ...
                      if(mysql_num_rows($rs1) > 0)
                   {
                    ...
                    $parent_id=$row1["id"];
                       ...
                       $value1['category_name']= strip_tags(trim($value['category_name']));
                       $value1['description']=htmlspecialchars(strip_tags($value['category_name']));
					   $value1['description'] = addslashes($value1['description']);
                       $this->loadmodel('Product_category_lang'); 
                       $this->Product_category_lang->create();
                       $this->Product_category_lang->save($value1);
                   }
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."' and parent_id=$parent_id");  
                   ...
                   if(mysql_num_rows($rs1) > 0)
                   {
                    ...
                    $fcatid=$row1["id"];
                       ...
                       $value1['status']=1;
                       $value1['category_name']= strip_tags(trim($value['category_name']));
					   $value1['category_name'] = addslashes($value1['category_name']);
                       $value1['description']=htmlspecialchars(strip_tags($value['category_name']));
					   $value1['description'] = addslashes($value1['description']);
                       $this->loadmodel('Product_category_lang'); 
                       $this->Product_category_lang->create();
    ...
    $price=$row["price"];
	$product_name=addslashes($row["product_name"]);
	$product_slug=addslashes($row["slug"]);
    $slug=$this->toAscii($department."-".$category_name."-".$product_name); 
    $cat_id=$fcatid;
	$offer_id = $row['offer_id'];
   ...
   // $rs2=mysql_query("select * from mc_products where category_id = '".$fcatid."' and slug = '".$slug."' and retailer_id = '".$retailer_id."'")or die(mysql_error());
    ...
    mysql_query("update mc_product_langs set 
	title='".$product_name."',
	description='".addslashes($row["product_description"])."',
	status=1 where product_id = '".$product_id."' and lang_id='".$lang."'")or die(mysql_error());
...
public function namshi(){
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."' and parent_id=$parent_id");  
	...
	$product_name=addslashes($row["product_name"]);
	$product_slug=$row["slug"];
    //$slug=$this->toAscii($product_name."-".$department."-".$category_name);
	$slug=$this->toAscii($department."-".$category_name.'-'.$product_name);
    $cat_id=$fcatid;
	$offer_id = $row['offer_id'];
	if($lang == 1){
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."' and parent_id=$parent_id");  
    ...
    $price=0;
    $price=$row["price"];
	$product_name=addslashes(trim($row["product_name"]));
	$product_slug=$row["slug"];
    $slug=$this->toAscii($department."-".$category_name.'-'.$product_name); 
    $cat_id=$fcatid;
	$offer_id = $row['offer_id'];
	...
	'".$product_id."',$lang,
	'".$product_name."',
	'".addslashes(trim($row["product_description"]))."',
	'','',1)")or die(mysql_error());
   ...
   $rs1=mysql_query("select * from mc_product_categories where slug = '".$slug."' and parent_id=$parent_id");  
	...
	if(mysql_num_rows($rs2) <= 0){
      ...
      mysql_query("insert into mc_products(slug,product_url,category_id,retailer_id,price,price_type,image_url,brand,status,deleted,created_date,offer_id) values(
	'".$slug."',
	'".addslashes($row["product_url"])."',
	'".$cat_id."','".$retailer_id."','".$price."','SAR',
	'".addslashes($row["image_url"])."','".$brand."',1,0,now(),'".$offer_id."')")or die(mysql_error());
	...
	'".$product_name."',
	'".addslashes($row["product_description"])."',
	'".trim($row["specification"])."','',1)")or die(mysql_error());
	...
	$brand = $row['brand'];
        ...
        mysql_query("update mc_products set 
	slug= '".$slug."',
	product_url='".addslashes($row["product_url"])."',
	category_id='".$cat_id."',
	retailer_id=$retailer_id,
	price='".$price."',
	price_type='SAR',
	...
	status=1,
	deleted=0,
	last_modified=now(),'".$offer_id."' where category_id = '".$cat_id."' and slug = '".$slug."' and retailer_id = '".$retailer_id."'")or die(mysql_error());
