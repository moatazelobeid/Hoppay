...
/*-----Compare Page------*/
  ...
  ini_set('max_execution_time','1000');
	...
	$slug1 = explode("-dept-",$slug);  
	//$parent_id = $slug1[0];
	if(isset($slug1[1]) && !empty($slug1[1])){
		$parent_id = $slug1[0];
		$slug = $slug1[1];
	}else{
		$slug = $slug1[0];
  ...
  $this->loadModel('Product_brand');
    ...
    switch ($this->params['type']) {
      case 'category':
			...
			if(empty($slug1[1])){ 
				$pid = $this->Product_category->findByslug($slug);
				$slug_nm = $this->Product_category->findByparent_id($pid['Product_category']['id']);
				$search_cat = $this->Product_category->find('first', 
										array('conditions'=>array(
											'Product_category.slug'=>$slug_nm['Product_category']['slug'],
											'Product_category.parent_id'=>$slug_nm['Product_category']['parent_id']
															  )
										)); 
			}
...
//print_r($brand);
     ...
     //print_r($this->metas_all);
			...
			$find_cat_id = $this->Product_category->find('first', array('fields'=>array('Product_category.id'),'conditions'=>array('Product_category.parent_id'=>$all_subid)));
            //echo "<pre>";print_r($find_cat_id); exit;
...
//echo $dtype;
      ...
      //echo   str_replace("-", "%",$slug);
          $brand=$this->Product_brand->findBySlug($slug); //echo "<pre>";print_r($brand);exit;
...
switch (@$array['type']) {
 ...
 function getCategoryDetails($cat,$search="",$type="",$sort=""){
   ...
   // echo count($merchants)."===";echo "<pre>";print_r($merchants);
      ?>       
                  ...
                  class="shop_by_btn" >
                   <?=$this->Ctrl->getPriceFormat(number_format($value['Product']['offer_price'],2))?>
                            ...
                            <?php } ?>
                                ...
                                <div class="small03_get_details">
								<?php // echo "<pre>";print_r($merchants);exit;  ?>
									<a href="<?=$this->webroot.$lang?>/products/<?=$products['Product']['id']?>-<?=$products['Product']['slug']?><?php if(count($merchants)>1){ ?>#one <?php } ?>">  
										<?php if(count($merchants)>1){ ?>
											<?=$this->Ctrl->getWord('compare_prices_from')?> <?=count($merchants)?> <?=$this->Ctrl->getWord('stores')?> 
										<?php }else{ ?> 
											<?=$this->Ctrl->getWord('more_info')?> 
										<?php } ?>
									</a>
								</div>
                                 </div> 
