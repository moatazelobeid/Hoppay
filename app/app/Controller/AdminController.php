     ...
     $this->loadmodel('Product_category'); 
      ...
      public function update_product_cat($id){
		...
		public function getAJAXCategorysChild($pid,$cno,$lang_id){
			...
			$cat_nm = array();
			foreach($product_categories as $val1){
				$cat_nm[$val1['Product_category']['id']] = $val1['Product_category_lang']['category_name'];
			}
			if(isset($this->request->data['Product_category']) && !empty($this->request->data['Product_category'])){
				$move_cat_id = $this->request->data['Product_category']; 
				if($move_cat_id['new_cat_id'] != $move_cat_id['old_cat_id']){
					$this->Product_category->recursive= -2;
					$records = $this->Product_category->find('all', array('conditions'=>array('Product_category.parent_id'=>$move_cat_id['old_cat_id'])));
					$record = count($records); 
					if($record > 0){ //echo "<pre>";print_r($records);//exit;
						foreach($records as $recc){ 
							$search_old_slug = $recc['Parent']['slug']."-".$recc['Product_category']['slug']; 
						}  
						$record22 = $this->Product_category->find('all', array('conditions'=>array('Product_category.id'=>$move_cat_id['new_cat_id'])));
						foreach($record22 as $rec22){
							$search_new_slug = $rec22['Parent']['slug']."-".$rec22['Product_category']['slug']; 
						} //echo $search_old_slug."--------".$search_new_slug;
						$search_res = $this->Product->query("SELECT id,slug FROM mc_products as Product WHERE slug LIKE '%".$search_old_slug."%'"); //echo count($search_res);exit;
						if(isset($search_res) && !empty($search_res)){
							foreach($search_res as $value){  
								$old_slug = $value['Product']['slug']; 
								$product_nm = explode($search_old_slug,$old_slug);  
								$new_slug = implode('',array($search_new_slug,$product_nm[1])); 
								$this->Product->query("UPDATE mc_products set slug = '".$new_slug."', category_id = ".$move_cat_id['new_cat_id']." WHERE id=".$value['Product']['id']);
							}
							$this->Product_category->unBindModel(array('hasMany' => array('Product')));
							$this->Product_category->delete($move_cat_id['old_cat_id']);
							$this->Product_category_lang->delete($move_cat_id['old_cat_id']);
						}
					}else{ 
						$this->Product_category->recursive= -2;
						$record1 = $this->Product_category->find('all', array('conditions'=>array('Product_category.id'=>$move_cat_id['old_cat_id'])));
						foreach($record1 as $rec){
							$search_old_slug = $rec['Parent']['slug']."-".$rec['Product_category']['slug']; 
						}
						$record2 = $this->Product_category->find('all', array('conditions'=>array('Product_category.id'=>$move_cat_id['new_cat_id'])));
						foreach($record2 as $rec2){
							$search_new_slug = $rec2['Parent']['slug']."-".$rec2['Product_category']['slug']; 
						}
						$search_res = $this->Product->query("SELECT id,slug FROM mc_products as Product WHERE slug LIKE '%".$search_old_slug."%'");
						$cat_new_id = $this->Product_category->find('first', array('conditions'=>array('Product_category.parent_id'=>$move_cat_id['new_cat_id'])));
						if(isset($search_res) && !empty($search_res)){
							foreach($search_res as $value){  
								$old_slug = $value['Product']['slug']; 
								$product_nm = explode($search_old_slug,$old_slug); 
								$new_slug = implode('',array($search_new_slug,$product_nm[1])); 
								$this->Product->query("UPDATE mc_products set slug = '".$new_slug."', category_id = ".$cat_new_id['Product_category']['id']." WHERE id=".$value['Product']['id']);
							} 
							$this->Product_category->unBindModel(array('hasMany' => array('Product')));
							$this->Product_category->delete($move_cat_id['old_cat_id']);
							$this->Product_category_lang->delete($move_cat_id['old_cat_id']);
					...
					$this->Session->setFlash('Category moved successfully.', 'default', array(), 'msg');
					$this->redirect('/admin/product_category_manager');
				}else{
					$this->Session->setFlash('You have selected the same category, please choose another category to move.', 'default', array(), 'msg');
					$this->redirect('/admin/product_category_manager');
				}
			}
			//$parents = $this->Product_category->getPath($pid);
			$this->set("catlist",$product_categories);
			$this->set("cat_nm",$cat_nm);
			$this->set("pid",$pid);
			$this->set("cno",$cno);
			$this->set("lang_id",$lang_id);
			$this->render('get_ajax_child_category');
