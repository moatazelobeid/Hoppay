function category(){
	...
	this.addHash=function(hashval,callback){
     ...
     this.getDataActive=function(callback){
	...
	this.checkTabs=function(select,slug){
	    ...
	    if (this.width <= 320 || this.width <= 480) {
	      //alert('dfg');
	        var section=$(select).parents('.cat_tab_content_area');
			$(select).parent().after(section.find('.cat_tab_contant'));
			console.log(this.counts);
			if( this.counts==0)
			{
				 jQuery('html, body').animate({
			    scrollTop: section.find('.cat_tab_contant').offset().top - 300
			    }, 800);
			}
