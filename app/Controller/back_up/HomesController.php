...
class HomesController extends AppController {
	...
	public $uses=array('Product_category','Product','Offer','Menu','Newsletter');
	...
	public function getoffers() 
	{
		...
		if($dbval['id']==0)
		{
		         if($dbval['text'])
				 {
					 ...
					 array('conditions'=>array('Product_lang.title LIKE'=>'%'.trim($dbval['text']).'%',
					 'Product.category_id !='=>'',
					 'Product.brand !='=>'','Product.offer_id !='=>'0'),'recursive' => 3));
				 }
				 else
				 {
				     $datas=$this->Product->Product_lang->find('all',
		...
		else 
		{
	      ...
	      $catids=Hash::extract($data, '{n}.Product_category.id');
		         if($dbval['text'])
				 {
					 ...
					 'Product.brand !='=>'',
					 'category_id'=>$catids,
					 'Product.offer_id !='=>'0'),'recursive' => 3));
				 }
				 else
				 {
				     $datas=$this->Product->Product_lang->find('all',
		...
		if(!empty($datas)){
          ...
          //  $this->Cookie->write('Search',array('search_id' =>$dbval['id'], 'text' => !empty($dbval['text'])?$dbval['text']:""));
			...
			foreach ( $datas as $key => $value) {
				if($value['Product']['Merchant']['User']['add_to_slider']!=0)
				{
					$todya = date('Y-m-d');
		    $start_date=date('Y-m-d', strtotime($value['Product']['Offer']['start_date']));
		    $end_date = date('Y-m-d', strtotime($value['Product']['Offer']['end_date']));
				...
				$view1.='<div class="ca-item">
						...
						<div class="ca-item-main">
							<div class="ca-offers"><del>'.$this->Ctrl->getPriceFormat(number_format($value['Product']['price'],2)).'</del> '.$this->Ctrl->getPriceFormat(number_format(($value['Product']['price']-($value['Product']['price']*$offer['Offer']['discount']/100)),2)).'<br><img src="'.$offer['Offer']['offer_image'].'" style="height:30px;float:left;margin-top:1px;width:auto" alt=""><span>'.$offer['Offer']['offer_desc'].'</span></div>
						  <div class="bottomtitle"><a  href="'.$this->webroot.$this->Ctrl->getLang() ."/products/".$value['Product']['id'].'-'.$value['Product']['slug'].'">'.((strlen($value['Product_lang']['title'])>10)?substr($value['Product_lang']['title'],0,10).'..':$value['Product_lang']['title']).'</a></div>
						  <a  href="'.$this->webroot.$this->Ctrl->getLang(). "/products/".$value['Product']['id'].'-'.$value['Product']['slug'].'"><img src="'.$image[0].'" alt=""></a>
						</div>
						</a>
					  </div>';
					}
